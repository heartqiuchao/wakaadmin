...
var App = function() {
	...
	var handleSidebarMenu = function() {
		...
		$('#sidebar-content ul > li > a').on('click', function (e) {  //一级菜单折叠控制

			if ($(this).next().hasClass('sub-menu') == false) {  //判断一级菜单项或二级菜单项
				e.preventDefault();
				if(!($(this).prop('href'))){
					return;
				}
				var target_url=$(this).attr('href');
		        var htmlobj=waka.ajax({url:$(this).attr('href'),async:false}, function (data) {
		        	//js
